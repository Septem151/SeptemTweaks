...
public abstract class EntryWorldGen implements IWorldGenerator, RegistryEntry {
	...
	public EntryWorldGen setDimensionGen(int dimensionId,
			WorldGenParams dimensionGenParams) {
	...
	public void generate(Random rand, int chunkX, int chunkZ, World world,
		...
		if (this.genParams.containsKey(dimensionId)) {
			...
			SeptemTweaks.LOG
					.error(String.format("Dimension %d was not loaded for %s WorldGen!",
							dimensionId, this.name));
		}
	}

	protected void generateBlock(World world, Random rand, int chunkX, int chunkZ,
			WorldGenParams params) {
		int veinSize = params.minVeinSize
				+ rand.nextInt(params.maxVeinSize - params.minVeinSize + 1);
		int heightRange = params.maxY - params.minY + 1;
		WorldGenMinable oreGen = new WorldGenMinable(this.block, veinSize,
				params.generateIn);
